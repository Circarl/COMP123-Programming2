...
class Atom
{
    ...
    //Constructor
    public Atom() { }
    public Atom(string name, string symbol, int proton, int neutron, double weight)
        => (Name, Symbol, Proton, Neutron, Weight) = (name, symbol, proton, neutron, weight);
    ...
    public static Atom Parse(string objectData)
    {
        ...
        return new Atom(data[0], data[4], proton, neutron, weight);
    }
    public override string ToString() => string.Format($"{Name} ({Symbol}) {Proton}, {Neutron} {Weight:F4}");
...
class Program
{
    //private const string firstItem_XML = "First Element.xml";
    private const string firstElement_JSON = "First_Element.json";
    private const string elements_JSON = "Entire_Elements.json";

    public static readonly List<Atom> elements = new List<Atom>();
    ...
    static void Main(string[] args)
    {
        ...
        foreach (Atom displayAtoms in elements)
            Console.WriteLine(displayAtoms);
        ...
        //Console.WriteLine(); //ExportFirstToXml(); //ImportFirstFromXml();

        Console.WriteLine();
        Console.WriteLine("Serialized First Atom");
        ExportFirstToJson();
        ImportFirstFromJson();

        Console.WriteLine();
        Console.WriteLine("Serialized All Atom");
        ExportToJson();
        ImportFromJson();
    ...
    {
        using (TextWriter writer = new StreamWriter(firstElement_JSON))
        {
            writer.Write(JsonSerializer.Serialize(elements[0]));
        }
    }
    //7 - Read the above file and display the item
    private static void ImportFirstFromJson()
    {
        using (TextReader reader = new StreamReader(firstElement_JSON))
        {
            Atom deserialized = JsonSerializer.Deserialize<Atom>(reader.ReadToEnd());
            Console.WriteLine($"Deserialized from {firstElement_JSON}: ");
    ...
    {
        using (TextWriter writer = new StreamWriter(elements_JSON))
        {
            writer.Write(JsonSerializer.Serialize(elements));
        }
    }
    //9 - Read the above file and display all the items
    private static void ImportFromJson()
    {
        using (TextReader reader = new StreamReader(elements_JSON))
        {
            List<Atom> deserialized = JsonSerializer.Deserialize<List<Atom>>(reader.ReadToEnd());
            Console.WriteLine($"Deserialized from {elements_JSON}:");
